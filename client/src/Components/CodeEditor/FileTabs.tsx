...
export const Tab = styled.div<any>`
  ...
  padding: 0 8px;
  background: ${(props) =>
    props.hover || props.isCurrent ? props.theme.colors.gray : "inherit"};
...
const TabComponent: React.FC<TabProps> = ({
  ...
  return (
    ...
    >
      <div className="title">
        <IconComponent />
        <Typography varient="h5" fontWeight={300}>
          {name}
        </Typography>
      </div>
      <div className="close">
        {currentFile === path || hover ? (
          <IoIosClose onClick={handleTabRemoved} />
        ) : null}
      </div>
    </Tab>
  );
