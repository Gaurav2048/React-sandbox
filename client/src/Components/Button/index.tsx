...
const Button: React.FC<ButtonProps> = ({
    variant, type, size, onClick, loading, children
}) => {
    if (variant === "contained") {
        return <ContainedButton {...sizes[size]} type={type} onClick={onClick}>
                {loading ? <Loader size={size} variant="primary" /> : null}{children}
            </ContainedButton>
    }
    
    if (variant === "outlined") {
        return <OutlinedButton {...sizes[size]} type={type} onClick={onClick}>
                {loading ? <Loader size={size} variant="primary" /> : null}{children}
            </OutlinedButton>
    }
